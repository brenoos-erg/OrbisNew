...
assert.match(seed, /attachmentRequiredMessage:\s*'Anexe o formulário RQ\.106 preenchido antes de enviar a solicitação\.'/, 'RQ_106 deve ter mensagem de anexo obrigatório')
assert.match(seed, /templateDownload:\s*\{\s*label:\s*'RQ\.106 - Pedido de Exclusao de Dependentes no Plano Medico e Odontológico\.docx',\s*url:\s*'\/templates\/solicitacoes\/RQ\.106 - Pedido de Exclusao de Dependentes no Plano Medico e Odontologico\.docx'/s, 'RQ_106 deve ter templateDownload configurado')
