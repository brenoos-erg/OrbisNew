...
const publishRoute = fs.readFileSync('src/app/api/documents/versions/[versionId]/publish/route.ts', 'utf8')
...
assert.match(approveRoute, /DOCUMENT_APPROVED/)
assert.doesNotMatch(approveRoute, /sendDocumentNotification\('DOCUMENT_QUALITY_REVIEW'/)
assert.match(approveRoute, /DOCUMENT_AWAITING_PUBLICATION/)
assert.match(publishRoute, /resolvePublicationNotificationEvent\(version\)/)
...
assert.match(publishRoute, /\.catch\(logDocumentNotificationFailure\(publicationEvent\)\)/)
