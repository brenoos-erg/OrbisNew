import {
  EXPERIENCE_EVALUATION_COMMENT_QUESTION,
  EXPERIENCE_EVALUATION_QUESTIONS,
} from "@/lib/experienceEvaluationQuestions";
import { onlyValidSolicitationStatuses } from "@/lib/solicitationStatuses";

export const EXPERIENCE_EVALUATOR_GROUP_NAME =
  "COORDENADORES_AVALIACAO_EXPERIENCIA";
export const EXPERIENCE_EVALUATION_TIPO_ID = "RQ_RH_103";
export const EXPERIENCE_EVALUATION_STATUS =
  "AGUARDANDO_AVALIACAO_GESTOR" as const;
export const EXPERIENCE_EVALUATION_FINALIZATION_STATUS =
  "AGUARDANDO_FINALIZACAO_AVALIACAO" as const;
export const EXPERIENCE_EVALUATION_CLOSED_STATUSES =
  onlyValidSolicitationStatuses(["CONCLUIDA", "FINALIZADA"]);
export const EXPERIENCE_EVALUATION_VISIBLE_STATUSES =
  onlyValidSolicitationStatuses([
    EXPERIENCE_EVALUATION_STATUS,
    EXPERIENCE_EVALUATION_FINALIZATION_STATUS,
    ...EXPERIENCE_EVALUATION_CLOSED_STATUSES,
  ]);
export const EXPERIENCE_EVALUATION_REQUIRED_FIELDS = [
  ...EXPERIENCE_EVALUATION_QUESTIONS.map((question) => question.field),
  EXPERIENCE_EVALUATION_COMMENT_QUESTION.field,
] as const;
