...
async function requireUser(action: Action, feature: string) {
  ...
  if (!allowed) {
    return {
      response: NextResponse.json(
        { error: "Acesso negado ao agendamento de salas.", requestId },
        { status: 403 },
      ),
    };
  }
...
export async function GET(req: Request) {
  const auth = await requireUser(
    Action.VIEW,
    FEATURE_KEYS.AGENDAMENTO_SALAS.VISUALIZAR,
  );
  ...
  if (room && !ROOMS.includes(room as MeetingRoomName))
    return NextResponse.json({ error: "Sala inválida." }, { status: 400 });
  if (status && !STATUSES.includes(status as MeetingRoomBookingStatus))
...
export async function POST(req: Request) {
  const auth = await requireUser(
    Action.CREATE,
    FEATURE_KEYS.AGENDAMENTO_SALAS.MARCAR,
  );
  if ("response" in auth) return auth.response;
  ...
  if (!title || !meetingType)
    return NextResponse.json(
      { error: "Informe título e tipo de reunião." },
      { status: 400 },
    );
  if (
    ...
    return NextResponse.json(
      { error: "Data/hora inicial deve ser menor que a final." },
      { status: 400 },
    );
  }
  ...
  if (duration > MAX_DURATION_MS)
    return NextResponse.json(
      { error: "Duração máxima de 8 horas." },
      { status: 400 },
    );
  ...
  const requester = requesterId
    ? await prisma.user.findFirst({
        where: { id: requesterId, status: "ATIVO" },
      })
    : auth.appUser;
  if (!requester)
    return NextResponse.json(
      { error: "Usuário solicitante não encontrado ou inativo." },
      { status: 400 },
    );
  ...
  const booking = await (prisma as any).meetingRoomBooking.create({
    data: {
      ...
      createdByName:
        auth.appUser.fullName || auth.appUser.login || auth.appUser.email,
    },
  });
  return NextResponse.json(
