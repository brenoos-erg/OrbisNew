...
import {
  ensureUniqueActiveIndexador,
  getPrismaP2000Message,
  getPrismaP2002Message,
  positionDataFromBody,
  positionSelect,
  withCurrentDocument,
} from "./positionFields";
        ...
        where: {
          ...
          ...(q
            ? {
                OR: [
                  { name: { contains: q } },
                  { indexador: { contains: q } },
                  { areaSector: { contains: q } },
                  { sectorProject: { contains: q } },
                  { cbo: { contains: q } },
                  {
                    documents: {
                      some: { indexador: { contains: q }, isCurrent: true },
                    },
                  },
                  { documents: { some: { indexador: { contains: q } } } },
                ],
              }
            : {}),
        },
...
export async function POST(request: Request) {
  try {
    ...
    const effectiveIndexador =
      body.extractedDocument?.indexador ?? body.indexador;
    if (
      body.extractedDocument?.indexador &&
      !String(body.indexador ?? body.extractedDocument.indexador).trim()
    )
      return NextResponse.json(
        {
          error:
            "Documento com indexador identificado exige código/indexador do cargo.",
        },
        { status: 400 },
      );
    await ensureUniqueActiveIndexador(prisma, effectiveIndexador);
    const created = await prisma.position.create({
      data: positionDataFromBody(body) as any,
      select: positionSelect,
    });
    ...
    const reloaded = await prisma.position.findUnique({
      where: { id: created.id },
      select: positionSelect,
    });
    return NextResponse.json(withCurrentDocument(reloaded ?? created), {
      status: 201,
    });
  } catch (error) {
    const p2000Message = getPrismaP2000Message(error);
    if (p2000Message)
      return NextResponse.json({ error: p2000Message }, { status: 400 });
    const p2002Message = getPrismaP2002Message(error);
    if (p2002Message)
      return NextResponse.json({ error: p2002Message }, { status: 400 });
    if (
      (error as Error)?.message ===
      "Já existe um cargo ativo com este código/indexador."
    )
      return NextResponse.json(
        { error: "Já existe um cargo ativo com este código/indexador." },
        { status: 409 },
      );
    if ((error as Error)?.message === "Data do documento inválida.")
      return NextResponse.json(
        { error: "Data do documento inválida." },
        { status: 400 },
      );
    if ((error as Error)?.message === "Usuário não autenticado")
      return NextResponse.json(
