...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!canOperationallyCancel || solicitation.solicitanteId === me.id) return NextResponse.json({ error: 'Você não possui permissão para recusar este cancelamento.' }, { status: 403 })
    const updated = await analyzeCancellationRequest({ solicitationId: id, actorId: me.id, actorName: me.fullName ?? me.id, justificativa, approve: false })
    return NextResponse.json({ ok: true, solicitation: updated })
  } catch (error: any) {
    if (error?.status) return NextResponse.json({ error: error.message }, { status: error.status })
    console.error('POST /api/solicitacoes/[id]/cancelamento/recusar error', error)
    return NextResponse.json({ error: 'Erro ao recusar cancelamento.' }, { status: 500 })
  }
}
