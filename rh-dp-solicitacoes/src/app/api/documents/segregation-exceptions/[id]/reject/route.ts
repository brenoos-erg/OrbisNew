...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (reason.length < 10) return NextResponse.json({ error: 'Motivo da rejeição da exceção é obrigatório e deve ter no mínimo 10 caracteres.' }, { status: 400 })
  ...
  if (item.requestedById === me.id) return NextResponse.json({ error: 'Solicitante não pode rejeitar a própria exceção.' }, { status: 403 })
  ...
  const decided = await prisma.documentSegregationException.updateMany({ where: { id, status: 'PENDING' }, data: { status: 'REJECTED', pendingKey: null, decidedById: me.id, decidedAt: new Date(), decisionReason: reason } })
  ...
  await prisma.documentAuditLog.create({ data: { documentId: item.documentId, versionId: item.versionId, userId: me.id, action: 'SEGREGATION_EXCEPTION_REJECTED', reason, metadata: { exceptionId: id, conflictType: item.conflictType } } })
  void sendDocumentNotification('SEGREGATION_EXCEPTION_REJECTED', { documentId: item.documentId, versionId: item.versionId, actorUserId: me.id }).catch((error) => console.error('SEGREGATION_EXCEPTION_REJECTED notification failed', error))
