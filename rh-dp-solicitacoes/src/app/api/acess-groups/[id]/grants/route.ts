...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const grant = await prisma.accessGroupGrant.upsert({
    where: { groupId_moduleId: { groupId: (await params).id, moduleId: mod.id } },
    create: { groupId: (await params).id, moduleId: mod.id, actions: actions || [] },
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.accessGroupGrant.delete({ where: { groupId_moduleId: { groupId: (await params).id, moduleId: mod.id } } })
