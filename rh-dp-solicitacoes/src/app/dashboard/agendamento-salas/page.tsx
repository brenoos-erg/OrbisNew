...
export default function MeetingRoomSchedulingPage() {
  ...
  const loadCurrentUser = async () => {
    const response = await fetch("/api/agendamento-salas/me", {
      cache: "no-store",
    });
  ...
  const loadUsers = async (search = userSearch) => {
    ...
    const response = await fetch(
      `/api/agendamento-salas/usuarios?${params.toString()}`,
      { cache: "no-store" },
    );
    const data = await response.json().catch(() => ({}));
  ...
  const loadBookingsForDate = async (
    targetDate = date,
    targetRoomFilter = roomFilter,
  ) => {
    const params = new URLSearchParams({ date: targetDate });
    if (targetRoomFilter) params.set("room", targetRoomFilter);
    const response = await fetch(
      `/api/agendamento-salas?${params.toString()}`,
      { cache: "no-store" },
    );
    const data = await response.json().catch(() => ({}));
  ...
  const activeBookings = useMemo(
    () => bookings.filter((booking) => booking.status === "AGENDADA"),
    [bookings],
  );
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (scheduleConflict) {
      setMessage(
        "Sala já reservada nesse horário. Ajuste a sala ou o horário antes de agendar.",
      );
      return;
    }
    setLoading(true);
    ...
    const response = await fetch("/api/agendamento-salas", {
      ...
      body: JSON.stringify({
        ...
        requesterId: scheduleForAnotherUser ? selectedUserId : currentUser?.id,
    ...
    if (response.ok) {
      setDate(form.date);
      setActiveTab("view");
      await loadBookingsForDate(form.date);
    }
  };
  ...
  const cancelBooking = async (id: string) => {
    ...
    if (response.ok) await loadBookingsForDate();
  };

  const copySummary = async (booking: Booking) => {
    const room =
      rooms.find((item) => item.key === booking.room)?.label ?? booking.room;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-[var(--muted-foreground)]">
              Controle das salas Ouro, Solar e Diamante com prevenção de
              conflitos.
            </p>
          </div>
      ...
      <div className="flex gap-2">
        <button
          className={`app-button-${activeTab === "form" ? "primary" : "secondary"}`}
          onClick={() => setActiveTab("form")}
        >
          Marcar reunião
        </button>
        <button
          className={`app-button-${activeTab === "view" ? "primary" : "secondary"}`}
      ...
      {activeTab === "form" ? (
        <form
          onSubmit={submit}
          className="grid gap-4 rounded-xl border border-[var(--border-subtle)] bg-[var(--card)] p-6 md:grid-cols-2"
        >
          ...
          <label className="space-y-1 text-sm font-medium">
            ...
            <select
              ...
              value={form.room}
              onChange={(e) =>
                setForm({ ...form, room: e.target.value as Room })
              }
            >
              {rooms.map((room) => (
          ...
          <label className="flex items-center gap-2 text-sm font-medium md:col-span-2">
            <input
              type="checkbox"
              checked={scheduleForAnotherUser}
              onChange={(e) =>
                handleScheduleForAnotherUserChange(e.target.checked)
              }
            />{" "}
            ...
            <>
              ...
              {selectedUser && (
                <p className="text-sm text-[var(--muted-foreground)] md:col-span-2">
                  {selectedUser.costCenterName
                    ? `Centro de custo: ${selectedUser.costCenterName}`
                    : ""}
                  {selectedUser.costCenterName && selectedUser.phone
                    ? " · "
                    : ""}
                  {selectedUser.phone ? `Telefone: ${selectedUser.phone}` : ""}
                </p>
              )}
            </>
          )}
          <label className="space-y-1 text-sm font-medium">
            ...
            <input className="app-input" readOnly value={form.requesterName} />
          </label>
          <label className="space-y-1 text-sm font-medium">
          ...
          {scheduleConflict && (
            <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 md:col-span-2">
              Sala já reservada neste horário: {scheduleConflict.title} (
              {timeFmt.format(new Date(scheduleConflict.startsAt))} -{" "}
              {timeFmt.format(new Date(scheduleConflict.endsAt))}).
            </div>
          )}
          <label className="space-y-1 text-sm font-medium md:col-span-2">
            Observações
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            {rooms.map((room) => {
              const roomBookings = bookings.filter(
                (booking) => booking.room === room.key,
              );
              const activeRoomBookings = activeBookings.filter(
                (booking) => booking.room === room.key,
              );
              const now = new Date();
              const current = activeRoomBookings.find(
                (booking) =>
                  new Date(booking.startsAt) <= now &&
                  new Date(booking.endsAt) > now,
              );
              const next = activeRoomBookings.find(
                (booking) => new Date(booking.startsAt) > now,
              );
              const status = current
                ? `Em uso até ${timeFmt.format(new Date(current.endsAt))}`
                : next
                  ? `Próxima reunião às ${timeFmt.format(new Date(next.startsAt))}`
                  : "Livre agora";
              return (
                <section
                  key={room.key}
                  className="rounded-xl border border-[var(--border-subtle)] bg-[var(--card)] p-4 shadow-sm"
                >
                  <div className="mb-3 flex items-center justify-between">
                    <h2 className="text-lg font-semibold">Sala {room.label}</h2>
                    ...
                    {roomBookings.map((booking) => (
                      ...
                      >
                        ...
                        <p>
                          ...
                          {timeFmt.format(new Date(booking.endsAt))} ·{" "}
                          {booking.meetingType} · {booking.status}
                        </p>
                        <p className="text-[var(--muted-foreground)]">
                          Agendado por: {booking.createdByName || "-"}
                        </p>
                        <p className="text-[var(--muted-foreground)]">
                          Responsável: {booking.requesterName || "-"}
                        </p>
                        <div className="mt-2 flex gap-2">
                          ...
                          {booking.createdById === currentUser?.id ? (
                            ...
                            <span className="text-xs text-[var(--muted-foreground)]">
                              Somente quem criou pode cancelar.
                            </span>
                          )}
                        </div>
                      </div>
