...
export default function AcaoPlanoAvulsoDetailClient({ planId, actionId }: { planId: string; actionId: string }) {
  ...
  async function load() {
    try {
     ...
     const payload: DetailPayload = data.item
      ...
      setOrigem(action.origem || 'PLANO AVULSO')
      setReferencia(action.referencia || payload.plan.numeroPlano || '')
  ...
  async function patchAction(partial: Record<string, unknown> = {}) {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/sst/planos-de-acao/${planId}/acoes/${actionId}`, {
        method: 'PATCH',
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-start gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div>
          <p className="text-sm uppercase text-slate-500">Ação do Plano Avulso</p>
          <h1 className="text-2xl font-bold text-slate-900">Ação #{item.action.id.slice(-6)}</h1>
          <p className="text-sm text-slate-600">Nº Plano: {item.plan.numeroPlano}</p>
        </div>
       <div className="ml-auto flex flex-wrap gap-2">
         ...
         <button type="submit" form="form-acao-plano-avulso" disabled={readOnly || saving} className="rounded-md bg-orange-500 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">{saving ? 'Salvando...' : 'Salvar/Atualizar'}</button>
          <button type="button" onClick={() => router.push(`/dashboard/sgi/qualidade/planos-de-acao/${planId}`)} className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700">Sair</button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_360px]">
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          {activeTab === 'dados' ? (
            <form id="form-acao-plano-avulso" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-4">
              <label className="block text-sm font-medium text-slate-700">Evidências textuais da ação
        ...
        <aside className="space-y-3">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 text-sm shadow-sm">
            ...
            <p className="mt-2"><Link href={`/dashboard/sgi/qualidade/planos-de-acao/${planId}`} className="text-orange-600 hover:underline">Voltar para ações do plano</Link></p>
          </div>
        </aside>
      </div>
    </div>
