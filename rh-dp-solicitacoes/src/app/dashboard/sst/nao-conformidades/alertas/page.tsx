...
export default function NcAlertasAdminPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true)
      const res = await fetch('/api/sst/nao-conformidades/alertas-config', { cache: 'no-store' })
  ...
  async function submit(e: FormEvent) {
    ...
    const res = await fetch('/api/sst/nao-conformidades/alertas-config', {
      method: 'PUT',
