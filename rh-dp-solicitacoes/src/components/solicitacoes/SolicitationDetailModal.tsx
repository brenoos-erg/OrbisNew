...
export function SolicitationDetailModal({
    ...
    // estado do fluxo de finalização RH -> DP
    ...
    // Só RQ_063 entra nesse fluxo de RH -> DP
    ...
    const canFinalizeRh = isSolicitacaoPessoal && !isFinalizadaOuCancelada
    ...
    async function handleFinalizarRh() {
        ...
        try {
            // tenta descobrir o nome/documento do candidato a partir dos campos do formulário
            const candidatoNome =
                (payloadCampos.nomeColaborador as string) ||
                (payloadCampos.nomeCandidato as string) ||
                'Novo colaborador'

            const candidatoDocumento =
            ...
            const res = await fetch(
                `/api/solicitacoes/${solicitationId}/finalizar-rh`,
                {
                    ...
                    body: JSON.stringify({
                        candidatoNome,
                        candidatoDocumento,
                        // dataAdmissaoPrevista, salario, cargo etc se quiser depois
                    }),
                },
            )
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="max-h-[90vh] w-full max-w-5xl overflow-auto rounded-lg bg-white shadow-xl">
                ...
                {/* Rodapé */}
                <div className="flex items-center justify-between border-t border-slate-200 px-5 py-3 gap-3">
                    {/* mensagens de erro/sucesso da finalização */}
                    <div className="text-xs">
                        {closeError && <p className="text-red-600">{closeError}</p>}
                        {closeSuccess && <p className="text-emerald-600">{closeSuccess}</p>}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        {canFinalizeRh && (
                            ...
                            >
                                {closing ? 'Finalizando...' : 'Finalizar (RH → DP)'}
                            </button>
                        )}
