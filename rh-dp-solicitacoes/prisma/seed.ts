...
async function main() {
 ...
 if (rhDepartment) {
      ...
      update: {
        ...
        schemaJson: {
          ...
          camposEspecificos: [
            ...
            {
              name: 'podeDivulgarVaga',
              label: 'Pode divulgar a vaga?',
              type: 'select',
              required: true,
              options: ['Sim', 'Não'],
              stage: 'solicitante',
            ...
            {
              name: 'observacaoSigiloDivulgacao',
              label: 'Observação sobre sigilo/divulgação',
              type: 'textarea',
              required: false,
              stage: 'solicitante',
              section: 'Informações básicas',
  ...
  if (qualidadeDepartment) {
    ...
    const changeManagementSchema = {
      meta: {
        indexador: 'RQ.QUA.148',
        revisao: '01',
        ...
        actionPlan: {
          enabled: true,
          linkedEntity: 'Solicitation',
          source: 'generic-action-plan',
          allowMultiple: true,
          fields: ['itemOrdem', 'acao', 'responsavel', 'dataInicial', 'dataFinal', 'status', 'evidencias'],
        },
        workflow: {
          statuses: ['ABERTA', 'AGUARDANDO_APROVACAO_GESTOR', 'DEVOLVIDA_PARA_AJUSTES', 'AGUARDANDO_ANALISE_QUALIDADE', 'AGUARDANDO_APROVACAO_DIRETORIA', 'APROVADA_PARA_IMPLEMENTACAO', 'EM_IMPLEMENTACAO', 'AGUARDANDO_VERIFICACAO_EFICACIA', 'ENCERRADA', 'CANCELADA', 'REPROVADA'],
          steps: [
            { key: 'areaSolicitante', label: 'Área Solicitante', role: 'solicitante', action: 'Abre a solicitação' },
            { key: 'gestorArea', label: 'Gestor da Área', role: 'gestor', action: 'Avalia, aprova e autoriza a implementação', canReturnWithJustification: true },
            { key: 'qualidadeSigAnalise', label: 'Qualidade / SIG', role: 'qualidade_sig', action: 'Avalia impactos no SGI e requisitos', canReturnWithJustification: true },
            { key: 'gerenteGeralDiretoria', label: 'Gerente Geral / Diretoria', role: 'diretoria', action: 'Realiza aprovação final', canRejectWithJustification: true },
            { key: 'qualidadeEficaciaEncerramento', label: 'Qualidade', role: 'qualidade', action: 'Verifica eficácia e encerra a solicitação', requiresEffectivenessBeforeClose: true },
          ],
        },
        closeRules: { requiresEffectivenessVerification: true },
        tabs: ['Identificação', 'Tipo e Classificação', 'Descrição da Mudança', 'Impactos e Riscos', 'Documentos Impactados', 'Planejamento', 'Plano de Ação', 'Aprovações', 'Verificação de Eficácia'],
      },
      camposEspecificos: [
        { name: 'numeroProtocolo', label: 'Número do protocolo', type: 'text', readOnly: true, source: 'system_protocol', section: 'Identificação' },
        { name: 'dataSolicitacao', label: 'Data da solicitação', type: 'date', readOnly: true, source: 'createdAt', section: 'Identificação' },
        { name: 'solicitanteCargoFuncao', label: 'Solicitante / Cargo/Função', type: 'text', readOnly: true, source: 'logged_user_position', section: 'Identificação' },
        { name: 'localAplicacao', label: 'Local da aplicação', type: 'text', required: true, section: 'Identificação' },
        { name: 'coordenadorSetorContrato', label: 'Coordenador do setor ou contrato', type: 'text', required: true, section: 'Identificação' },
        { name: 'areaContrato', label: 'Área/Contrato', type: 'cost_center', required: true, section: 'Identificação' },
        { name: 'classificacaoMudanca', label: 'Classificação da mudança', type: 'select', required: true, options: ['Permanente', 'Temporária', 'Emergencial'], section: 'Tipo e Classificação' },
        { name: 'prioridadeMudanca', label: 'Prioridade', type: 'select', required: true, options: ['Baixa', 'Média', 'Alta', 'Crítica'], section: 'Tipo e Classificação' },
        { name: 'confidencialidadeMudanca', label: 'Sigilosa / Não Sigilosa', type: 'select', required: true, options: ['Sigilosa', 'Não Sigilosa'], section: 'Tipo da Mudança' },
        { name: 'tiposMudanca', label: 'Tipos da mudança', type: 'multi_select', required: true, options: ['Técnica', 'Processos', 'Materiais', 'Projetos e Construção', 'Ambiente de Trabalho', 'Equipamentos e/ou Instalações', 'Procedimentos Operacionais', 'Matérias-primas e Insumos Diversos', 'Infraestrutura', 'Tecnológica', 'Organizacional', 'Requisito Legal', 'Cliente / Contrato', 'Outras'], section: 'Tipo da Mudança' },
        { name: 'especificarOutraMudanca', label: 'Especificar outra mudança', type: 'textarea', requiredWhen: { field: 'tiposMudanca', includes: 'Outras' }, section: 'Tipo da Mudança' },
        { name: 'mudancaProposta', label: 'Mudança proposta', type: 'textarea', required: true, section: 'Descrição da Mudança' },
        { name: 'descricaoResumidaMudancaProposta', label: 'Descrição resumida da mudança proposta', type: 'textarea', required: true, section: 'Descrição da Mudança' },
        { name: 'justificativaMudanca', label: 'Justificativa da mudança', type: 'textarea', required: true, section: 'Descrição da Mudança' },
        { name: 'resultadoEsperado', label: 'Resultado esperado', type: 'textarea', required: true, section: 'Descrição da Mudança' },
        { name: 'dataPrevistaImplantacao', label: 'Data prevista para implantação', type: 'date', required: true, section: 'Descrição da Mudança' },
        { name: 'areasImpactadas', label: 'Áreas Impactadas', type: 'multi_select', required: true, options: ['Qualidade', 'SST', 'Meio Ambiente', 'Operação', 'Recursos Humanos', 'Cliente', 'Contrato', 'Patrimônio', 'Outros'], section: 'Impactos e Riscos' },
        { name: 'especificarOutrasAreasImpactadas', label: 'Especificar outras áreas impactadas', type: 'textarea', requiredWhen: { field: 'areasImpactadas', includes: 'Outros' }, section: 'Impactos e Riscos' },
        { name: 'tiposImpactosRiscos', label: 'Tipos de impactos/riscos associados', type: 'multi_select', options: ['Riscos à Saúde / Segurança', 'Compliance', 'Riscos Mecânicos', 'Aspectos Ambientais', 'Riscos de Operação', 'Manutenabilidade', 'Outros'], section: 'Impactos e Riscos' },
        { name: 'especificarOutrosRiscos', label: 'Especificar outros riscos', type: 'textarea', requiredWhen: { field: 'tiposImpactosRiscos', includes: 'Outros' }, section: 'Impactos e Riscos' },
        { name: 'matrizImpactoRisco', label: 'Matriz de perguntas de impacto e riscos', type: 'risk_matrix', responseOptions: ['SIM', 'NAO', 'NA'], detailField: 'detalheComplemento', questions: ['Pessoas serão expostas ou afetadas com as mudanças?', 'A mudança gera algum aspecto ambiental?', 'A mudança gera perigo ou risco de SST?', 'Existem requisitos contratuais aplicáveis?', 'Há necessidade de comunicação externa?', 'Há necessidade de treinamento?', 'Há necessidade de revisão documental?', 'Há necessidade de desenvolvimento ou revisão de projeto?', 'A mudança impacta clientes?', 'A mudança exige novos recursos ou investimentos?', 'Há necessidade de atualização de APR, LAP ou documentos do SGI?'], section: 'Impactos e Riscos' },
        { name: 'documentosImpactadosTipos', label: 'Documentos Impactados', type: 'multi_select', options: ['Procedimento', 'Formulário', 'LAP', 'APR', 'Plano', 'Matriz de Treinamento', 'Outro'], section: 'Documentos Impactados' },
        { name: 'especificarOutroDocumento', label: 'Especificar outro documento', type: 'textarea', requiredWhen: { field: 'documentosImpactadosTipos', includes: 'Outro' }, section: 'Documentos Impactados' },
        { name: 'documentosEnvolvidos', label: 'Documentos envolvidos', type: 'textarea', section: 'Documentos Impactados' },
        { name: 'estrategiaImplementacao', label: 'Estratégia de implementação', type: 'textarea', section: 'Planejamento' },
        { name: 'equipeMultidisciplinar', label: 'Equipe Multidisciplinar', type: 'repeater', allowMultiple: true, suggestedRoles: ['Solicitante da mudança', 'Aprovador da mudança', 'Executante da mudança', 'Representante do SIG', 'Representante da gerência ou coordenação', 'Responsável da área impactada'], fields: ['funcaoAnalise', 'nomeCompletoFuncao', 'confirmacaoDigital'], section: 'Planejamento' },
        { name: 'recursosHumanos', label: 'Recursos humanos', type: 'textarea', section: 'Planejamento' },
        { name: 'recursosMateriais', label: 'Recursos materiais', type: 'textarea', section: 'Planejamento' },
        { name: 'recursosFinanceiros', label: 'Recursos financeiros', type: 'textarea', section: 'Planejamento' },
        { name: 'recursosTecnologicos', label: 'Recursos tecnológicos', type: 'textarea', section: 'Planejamento' },
        { name: 'planoComunicacao', label: 'Plano de comunicação', type: 'textarea', section: 'Planejamento' },
        { name: 'planoTreinamentos', label: 'Plano de treinamentos', type: 'textarea', section: 'Planejamento' },
        { name: 'planoAcaoMudanca', label: 'Plano de Ação', type: 'action_plan', allowMultiple: true, linkedEntity: 'Solicitation', fields: ['itemOrdem', 'acao', 'responsavel', 'dataInicial', 'dataFinal', 'status', 'evidencias'], section: 'Plano de Ação' },
        { name: 'liberacaoImplementacao', label: 'Liberação para Implementação', type: 'approval_grid', approvals: ['Diretor ou gerente de contrato', 'Coordenador do setor ou contrato', 'Responsável da área impactada', 'SIG / Qualidade'], fields: ['nome', 'usuario', 'dataHora', 'decisao', 'observacaoJustificativa'], section: 'Aprovações' },
        { name: 'eficaciaResultadoEsperado', label: '9.1 - A mudança atingiu o resultado esperado?', type: 'select', options: ['Sim', 'Parcialmente', 'Não'], requiredForClosure: true, section: 'Verificação de Eficácia' },
        { name: 'eficaciaImpactosNaoPrevistos', label: '9.2 - Foram identificados impactos não previstos?', type: 'select', options: ['Sim', 'Não'], requiredForClosure: true, section: 'Verificação de Eficácia' },
        { name: 'detalharImpactosNaoPrevistos', label: 'Detalhar impactos não previstos', type: 'textarea', requiredWhen: { field: 'eficaciaImpactosNaoPrevistos', equals: 'Sim' }, section: 'Verificação de Eficácia' },
        { name: 'eficaciaControlesImplementados', label: '9.3 - Os controles implementados foram eficazes?', type: 'select', options: ['Sim', 'Não'], requiredForClosure: true, section: 'Verificação de Eficácia' },
        { name: 'eficaciaAcaoComplementar', label: '9.4 - Foi necessária ação complementar?', type: 'select', options: ['Sim', 'Não'], requiredForClosure: true, allowComplementaryActionPlan: true, section: 'Verificação de Eficácia' },
        { name: 'eficaciaResultadoFinal', label: '9.5 - Resultado final', type: 'select', options: ['Eficaz', 'Parcialmente eficaz', 'Ineficaz'], requiredForClosure: true, section: 'Verificação de Eficácia' },
        { name: 'responsavelAvaliacaoEficacia', label: 'Responsável pela avaliação', type: 'text', requiredForClosure: true, section: 'Verificação de Eficácia' },
        { name: 'dataAvaliacaoEficacia', label: 'Data da avaliação', type: 'date', requiredForClosure: true, section: 'Verificação de Eficácia' },
        { name: 'observacoesEficacia', label: 'Observações da eficácia', type: 'textarea', section: 'Verificação de Eficácia' },
    ...
    await prisma.tipoSolicitacao.upsert({
      ...
      update: {
        ...
        descricao: 'Solicitação de gestão de mudanças da Qualidade / SGI conforme RQ.QUA.148 Rev.01.',
        schemaJson: changeManagementSchema,
        updatedAt: new Date(),
      },
      create: {
        id: 'RQ_QUA_148',
